...
const Deposit = () => {
  ...
  // Fetch QR and UPI ID from API
  useEffect(() => {
    const fetchQr = async () => {
      try {
        const res = await axiosInstance.get("https://example.com/api/get-qr"); // <-- replace with real QR API
        // axios.get me response data res.data me milta hai
        setQr(res.data.qr_url);
    ...
    >
      <div className="w-full max-w-md bg-white shadow-xl rounded-2xl p-6">
        <h2 className="text-xl font-semibold text-red-600 text-center border-b pb-2">
          Welcome to Matka Mela
        </h2>

        <div className="bg-gray-50 p-3 rounded-lg mt-4 space-y-2">
          <p className="text-sm">Name: <span className="font-medium">User</span></p>
        ...
        <div className="text-center mt-4">
          {qr ? (
            <img src={`${backUrl}/uploads/qrscanner.jpg`} alt="QR Code" className="mx-auto w-48 h-48" />
